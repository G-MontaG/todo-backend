...
class CheckTokenMiddleware extends BaseMiddleware {
    ...
    private verifyAuthToken(data: { token: string, payload?: any, isExist: boolean }) {
        return new Promise((resolve, reject) => {
            const additionalRules = (payload: any) => {
                if (payload) {
                ...
                jwt.verify(data.token, publicKey, {
                    ...
                    if (err) {
                        if (err.name === 'TokenExpiredError') {
                            // Refresh token logic
                            const decodedPayload: any = jwt.decode(data.token);
    ...
    private getRefreshTokenFromDB(payload: any): Promise<IUserDocument> {
        return User.findById(payload.iss).exec();
    }
    ...
    private setNewAccessTokenToResponseHeaders(data:
        { user: IUserDocument, payload: any, result: boolean, accessToken: string, xsrfToken: string }) {
        ...
        return new Promise((resolve) => {
            if (data.exp) {
                redisConnection.client.set(
                    'ACCESS-TOKEN:' + data.token, JSON.stringify(data.payload),
                    'EX',
...
export function checkTokenMiddleware(req: express.Request, res: express.Response, next: express.NextFunction) {
    new CheckTokenMiddleware(req, res, next).middleware();
}
